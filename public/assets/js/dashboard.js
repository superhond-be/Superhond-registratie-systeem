/* dashboard.js — UI logica */
import {loadDB, saveDB, downloadJSON} from './storage.js';
import {
  computeEndISO, toMapsUrl, createLesson, addLesson, updateLesson, deleteLesson,
  listLessons, createNotice, addNotice, listNotices, generateSeries, buildAgenda, getLesson
} from './models.js';
...
agendaStart.value = new Date().toISOString().slice(0,10);
...
els.btnReset.addEventListener('click', ()=>{ formLes.reset(); els.id.value=''; });
...
formLes.addEventListener('submit', (e)=>{
  ...
  const payload = {
    ...
    packageId: null, seriesId: null
  };
  if(!payload.title || !payload.startISO) { alert('Titel en start zijn verplicht.'); return; }
...
function renderLessen(){
  ...
  list.forEach(l=>{
    ...
    li.innerHTML = `
      <span><strong>${escapeHTML(l.title)}</strong>
      ...
      <button data-act="del" class="danger">Verwijder</button>
    `;
    ...
    li.querySelector('[data-act="del"]').addEventListener('click', ()=>{
      if(confirm('Les verwijderen?')){ deleteLesson(l.id); renderLessen(); renderAgenda(); }
    });
...
elsR.btnGen.addEventListener('click', ()=>{
  ...
  if(!data.pakNaam || !data.reeksNaam || !data.startDate){ alert('Vul pakket, reeks en start in.'); return; }
  ...
  renderLessen();
  renderAgenda();
  alert(`Aangemaakt: ${gen.lessons.length} lessen in reeks "${gen.series.name}" van pakket "${gen.pkg.name}".`);
});
...
function renderAgenda(){
  ...
  const from = new Date(agendaStart.value+'T00:00');
  ...
  items.forEach(it=>{
    ...
    li.innerHTML = `
      ...
      ${it.type==='les' && it.location?.name ? `<div class="meta">📍 <a href="${it.location.mapsUrl||'#'}" target="_blank">${escapeHTML(it.location.name)}</a></div>`:''}
...
function ensureDemoSeed(){
  const db = loadDB();
  if(db.lessons.length===0 && db.notices.length===0){
    // drop a tiny seed for a nice first view
    ...
    db.notices.push(createNotice({
      title:'Welkom!',
      message:'Agenda en mededelingen zijn gekoppeld ✔️',
