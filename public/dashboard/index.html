<!doctype html>
<html lang="nl">
<head>
  <meta charset="utf-8" />
  <meta name="viewport" content="width=device-width,initial-scale=1" />
  <title>Superhond — Dashboard</title>
  <meta name="superhond-exec" content="https://script.google.com/macros/s/AKfycbyxPiFzgCcMplcC5kyFgVPHcIIjDXcQoWdPMEvI2zj-aP6ud8mG49xicSHd8SUcG22sPw/exec">
  <meta name="superhond-branch" content="Ontwikkeling">
  <meta name="superhond-version" content="0.27.6-debug">

  <link rel="stylesheet" href="../css/style.css?v=0.27.5" />
  <link rel="stylesheet" href="../css/superhond.css?v=0.27.5" />

  <style>
    body.dashboard-page #topbar,
    body.dashboard-page #topbar .topbar-inner {
      background: var(--accent, #f4c400) !important;
      color: #000 !important;
    }
  </style>
</head>
<body class="dashboard-page">
  <header id="topbar"></header>

  <main class="container" role="main">
    <h1 class="page-title">Hoofdmenu</h1>
    <nav class="tiles" aria-label="Hoofdmenu">
      <a class="tile" href="../klanten/"><span>👤</span> Klanten</a>
      <a class="tile" href="../honden/"><span>🐶</span> Honden</a>
      <a class="tile" href="../lessen/"><span>📅</span> Lessen</a>
      <a class="tile" href="../mededeling/"><span>📢</span> Mededelingen</a>
      <a class="tile" href="../trainers/"><span>🧑‍🏫</span> Trainers</a>
      <a class="tile" href="../rapporten/"><span>📈</span> Rapporten</a>
      <a class="tile" href="../instellingen/"><span>⚙️</span> Instellingen</a>
      <a class="tile" href="../over/"><span>ℹ️</span> Over</a>
    </nav>
  </main>

  <footer id="footer" class="footer"></footer>

  <script type="module">
    import { SuperhondUI } from '../js/layout-debug.js';
    import '../js/dashboard.js?v=0.27.5';

    console.log('[dashboard debug] calling mount');
    SuperhondUI.mount({ title: 'Superhond', icon: '🐾', home: true });
  </script>
</body>
</html>
