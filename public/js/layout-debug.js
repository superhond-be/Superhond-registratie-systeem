...
function renderTopbar(container, opts, online) {
  ...
  // in debug version, we always append version
...
async function mount(opts = {}) {
  ...
  console.log('[debug] DOM ready, document.body.class:', document.body.className);

  const path = location.pathname.replace(/\/+$/, '');
  let isDash = /\/dashboard$/.test(path) || /\/dashboard\/index\.html$/.test(path) || opts.home === true;
  if (!isDash && (path === '/index.html' || path === '/')) {
    isDash = true;
  }
  console.log('[debug] isDash determined as:', isDash);

  document.body.classList.toggle('dashboard-page', isDash);
  document.body.classList.toggle('subpage', !isDash);

  const online = await pingExec().catch(e => {
    console.warn('[debug] pingExec failed:', e);
    return false;
  });
  console.log('[debug] pingExec returned online =', online);
