...
function fmtDateISOtoLocal(iso){ // 'YYYY-MM-DD' → 'DD/MM/YYYY'
  const s = String(iso || '').trim();
  if (!/^\d{4}-\d{2}-\d{2}$/.test(s)) return s || '';
  const [y,m,d] = s.split('-');
  return `${d}/${m}/${y}`;
}
function fmtTimeToHHMM(t){ // accepteert 'HH:MM' of 'H:MM' → 'HH:MM'
...
function normalizeRow(row){
  // Headers kunnen variëren (saveLes ondersteunt aliassen)
